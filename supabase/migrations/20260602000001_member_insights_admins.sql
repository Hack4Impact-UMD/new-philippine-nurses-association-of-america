-- Loosen member_insights() so chapter admins and above can read the aggregate
-- charts on the Members page (previously national_admin only). The blob is an
-- org-wide rollup with no per-member rows, so region / chapter admins seeing it
-- exposes aggregate counts only — matching the new UI gate (useIsAdmin).
...
-- Grant unchanged; the function itself raises if the caller isn't an admin.
