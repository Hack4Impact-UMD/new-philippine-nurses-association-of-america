...
async function runBackfill() {
  ...
  for (const doc of eventsSnap.docs) {
    ...
    if (data.eventType === undefined) updates.eventType = "conference";
    if (data.eventSubtype === undefined) updates.eventSubtype = "in_person";
    if (data.defaultHours === undefined) updates.defaultHours = 0;
    if (data.attendedCount === undefined) updates.attendedCount = 0;
    if (data.contactHours === undefined) updates.contactHours = 0;
  ...
  for (const doc of attendeesSnap.docs) {
    const data = doc.data();
    const updates: Record<string, unknown> = {};
    if (data.source === undefined) updates.source = "wildapricot";
    if (data.attended === undefined) updates.attended = false;
    if (data.hours === undefined) updates.hours = 0;
    if (data.memberId === undefined) updates.memberId = String(data.contactId ?? "");
    if (Object.keys(updates).length > 0) {
  ...
  if (attBatchCount > 0) await attBatch.commit();
