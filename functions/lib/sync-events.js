        ...
        try {
            ...
            for (const [id, incoming] of waRegs) {
                ...
                if (!existing) {
                    ...
                    eventBatch.set(eventRef.collection("attendees").doc(id), incoming, { merge: true });
                    eventBatchCount++;
                    updated++;
                }
                await commitIfFull();
            ...
            eventBatch.update(eventRef, {
                ...
                ...(attendedDelta !== 0 && { attendedCount: firestore_1.FieldValue.increment(attendedDelta) }),
                ...(hoursDelta !== 0 && { contactHours: firestore_1.FieldValue.increment(hoursDelta) }),
